...
func TestGetRevenueTrendUseCase_Execute(t *testing.T) {
	t.Run("when no dates provided then defaults to last 30 days", func(t *testing.T) {
		// Arrange
		ctx := context.Background()
		shopID := 1
		filters := models.MetricsFilters{}
		expected := []models.RevenueTrendPoint{{Date: "2024-06-01", Revenue: 100}}

		serviceMock := mocks.NewMetricsService(t)
		serviceMock.EXPECT().
			GetRevenueTrend(mock.Anything, shopID,
		...
		uc := NewGetRevenueTrendUseCase(serviceMock)
	...
	t.Run("when DateFrom and DateTo provided then uses them", func(t *testing.T) {
		...
		expected := []models.RevenueTrendPoint{{Date: "2024-01-15", Revenue: 200}}

		serviceMock := mocks.NewMetricsService(t)
		serviceMock.EXPECT().
			GetRevenueTrend(mock.Anything, shopID, dateFrom, dateTo, "UTC").
			Return(expected, nil)
	...
	t.Run("when only DateFrom provided then uses it with default to", func(t *testing.T) {
		...
		expected := []models.RevenueTrendPoint{}

		serviceMock := mocks.NewMetricsService(t)
		serviceMock.EXPECT().
			GetRevenueTrend(mock.Anything, shopID, dateFrom, mock.AnythingOfType("time.Time"), "UTC").
			Return(expected, nil)
	...
	t.Run("when only DateTo provided then uses default from with DateTo", func(t *testing.T) {
		...
		serviceMock := mocks.NewMetricsService(t)
		serviceMock.EXPECT().
			GetRevenueTrend(mock.Anything, shopID, mock.AnythingOfType("time.Time"), dateTo, "UTC").
			Return(expected, nil)
	...
	t.Run("when service errors then propagates error", func(t *testing.T) {
		...
		serviceMock := mocks.NewMetricsService(t)
		serviceMock.EXPECT().
			GetRevenueTrend(mock.Anything, shopID, mock.Anything, mock.Anything, "UTC").
	...
	t.Run("when timezone provided then parses correctly", func(t *testing.T) {
		...
		serviceMock := mocks.NewMetricsService(t)
		serviceMock.EXPECT().
			GetRevenueTrend(mock.Anything, shopID, mock.Anything, mock.Anything, "America/Argentina/Buenos_Aires").
			Return(expected, nil)
